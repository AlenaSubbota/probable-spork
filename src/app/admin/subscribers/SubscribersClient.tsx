      ...
      <section className="market-section">
        ...
        {pending.length === 0 ? (
          <div className="empty-state" style={{ padding: 18 }}>
            <p>
              Новых заявок, ждущих твоего одобрения, нет.
              Если подключён Boosty-автосинк — подписки читателей
              подтверждаются автоматически и сразу попадают в «Активных»,
              здесь появляются только те, что нужно одобрить руками
              (монетные покупки и подписки от платформ без автосинка).
            </p>
          </div>
        ) : (
                ...
                <article key={c.id} className="application-card">
                  ...
                  <div className="claim-details">
                    ...
                    <div className="claim-hint">
                      {isCoinsClaim ? (
                        <>
                          <strong>Ручная сверка:</strong> открой свою панель{' '}
                          {PROVIDER_LABEL[c.provider] ?? c.provider} (или
                          банковскую выписку), найди платёж/донат от этого
                          читателя на сумму <strong>{c.coins_amount ?? 0} ₽</strong>
                          {' '}с кодом <code>{c.code}</code> в комментарии.
                          Сумма должна совпадать — если читатель заявил 300, а
                          оплатил 100, отклоняй (или одобряй меньшую сумму
                          вручную через коин-транзакцию). Одобрение зачислит
                          {' '}{c.coins_amount ?? 0} монет на его кошелёк у тебя.
                        </>
                      ) : (
                        'Сверь ник и код — если у тебя подключён Boosty-автосинк, такие заявки обычно одобряются автоматически и здесь не появляются. Эта значит, что email не совпал с твоим списком Boosty-подписчиков: проверь вручную перед одобрением.'
                      )}
                    </div>
                  </div>
