...
export default function QuoteCollection({ initial }: Props) {
  ...
  // Публичные цитаты появляются в блоке «Цитата дня» на главной и в
  // карточке новеллы. Перед включением публичности — обязательное
  // предупреждение про спойлеры (мы не модерируем содержание цитат
  // автоматически, но цитата с явным спойлером может быть удалена
  // админом по жалобе читателя).
  const handleTogglePublic = async (id: number, next: boolean) => {
    if (next) {
      const ok = window.confirm(
        '⚠ Цитата станет видна на главной и в карточке новеллы.\n\n' +
          'Цитаты с прямыми спойлерами (имена убийц, ключевые повороты, ' +
          'смерти героев) — недопустимы и могут быть удалены модерацией. ' +
          'Если это кусок-настроение, фраза без контекста — публикуй ' +
          'спокойно.\n\nСделать публичной?'
      );
      if (!ok) return;
    }
